...
function doGet(e) {
  try {
    ...
    try {
      ...
      var maskedEmail = emailStr
        ? emailStr.replace(/(^.).*(@.*$)/, "$1***$2")
        : "";
      console.log(
        "[doGet] action=",
        p.action,
      ...
      console.warn("[doGet] log failed", logErr);
    }
    ...
    function loginWithPassword_(email, password) {
      try {
        ...
        if (!otpRes || otpRes.success !== true) {
          console.warn(
            "[loginWithPassword] sendOtp_ failed:",
            otpRes && otpRes.error ? String(otpRes.error) : "(no error)"
          );
          return {
            success: false,
            error: otpRes && otpRes.error ? otpRes.error : "Failed to send OTP",
          };
        }
    ...
    function verifyLogin_(email, code) {
      try {
        ...
        if (!ver || ver.success !== true) {
          ...
          return {
            success: false,
            error: ver && ver.error ? ver.error : "OTP verification failed",
          };
        }
    ...
    if (p.action === "sendOtp") {
      try {
        ...
        var masked2 = emailStr2
          ? emailStr2.replace(/(^.).*(@.*$)/, "$1***$2")
          : "";
    ...
    if (p.action === "verifyOtp") {
      try {
        ...
        var masked3 = emailStr3
          ? emailStr3.replace(/(^.).*(@.*$)/, "$1***$2")
          : "";
    ...
    if (p.action === "getUserProfile") {
      try {
        ...
        var masked4 = emailStr4
          ? emailStr4.replace(/(^.).*(@.*$)/, "$1***$2")
          : "";
...
function getLoginMasterSheet_() {
  ...
  if (sh.getLastRow() === 0) {
    ...
    var header = sh
      .getRange(1, 1, 1, Math.max(6, sh.getLastColumn()))
      .getValues()[0];
...
function findGmailRow_(sh, gmail) {
  ...
  var target = String(gmail || "")
    .trim()
    .toLowerCase();
  for (var i = 1; i < vals.length; i++) {
    var g = String(vals[i][3] || "")
      .trim()
      .toLowerCase(); // Col D index 3 (0-based)
...
function startSignup_(email, password, name) {
  try {
    ...
    if (DEBUG_STORE_PLAINTEXT) {
      stagedPayload.pwdPlain = String(password);
    }
...
function getTrades_(sheetName) {
  ...
  if (!values || values.length < 2) {
    console.warn(
      "[Trades] Insufficient data - need at least 1 row after headers"
    );
    return json_([]);
  }
  ...
  var iEntryDate = findHeader(headers, [
    "entry date",
    "entrydate",
    "timestamp",
    "date",
  ...
  var iExitDate = findHeader(headers, [
    "exit date",
    "exitdate",
    "exit",
    "close date",
  ...
  var iInstrument = findHeader(headers, [
    "instrument",
    "symbol",
    "ticker",
    "scrip",
  ...
  var iEntry = findHeader(headers, [
    "entry",
    "entry price",
    "price",
    "buy price",
  ]);
  var iExit = findHeader(headers, [
    "exit",
    "exit price",
    "sell price",
    "close",
  ...
  var iRiskRs = findHeader(headers, [
    "risk in rs",
    "risk (₹)",
    "risk rs",
    "risk amount",
  ]);
  var iRiskPct = findHeader(headers, [
    "risk in %",
    "risk (%)",
    "risk percent",
    "risk %",
  ...
  console.log("[Trades] Resolved indices:", {
    iEntryDate,
    iExitDate,
    iInstrument,
    iSide,
  ...
  var trades = rows.filter(nonEmptyRow_).map(function (r, i) {
    console.log(
      "[Trades] Processing row",
      i + 2,
      ":",
      r.slice(0, 5).join(", "),
      "..."
    ...
    var side = action_(
      iSide >= 0 ? r[iSide] : idx["side"] != null ? r[idx["side"]] : ""
    );
    var entry = num_(
      iEntry >= 0 ? r[iEntry] : idx["Entry"] != null ? r[idx["Entry"]] : 0
    );
    var exit = num_(
      iExit >= 0 ? r[iExit] : idx["Exit"] != null ? r[idx["Exit"]] : 0
    );
    var qty = num_(
      iQty >= 0 ? r[iQty] : idx["Qty"] != null ? r[idx["Qty"]] : 0
    );
    var pnl = num_(
      iPnl >= 0 ? r[iPnl] : idx["P&L"] != null ? r[idx["P&L"]] : 0
    );
    var notes = String(
      (iNotes >= 0 ? r[iNotes] : idx["notes"] != null ? r[idx["notes"]] : "") ||
        ""
    );
    var strategy = String(
      (iStrategy >= 0
    ...
    var stoploss =
      iStop >= 0
        ? num_(r[iStop])
        : idx.hasOwnProperty("Stoploss")
        ? num_(r[idx["Stoploss"]])
        : 0;
    var takeprofit =
      iTp >= 0
        ? num_(r[iTp])
        : idx.hasOwnProperty("takeprofit")
        ? num_(r[idx["takeprofit"]])
        : idx.hasOwnProperty("Takeprofit")
        ? num_(r[idx["Takeprofit"]])
        : 0;
    var riskInRs =
      iRiskRs >= 0
        ? num_(r[iRiskRs])
        : idx.hasOwnProperty("Risk in rs")
        ? num_(r[idx["Risk in rs"]])
        : 0;
    var riskInPct =
      iRiskPct >= 0
        ? num_(r[iRiskPct])
        : idx.hasOwnProperty("Risk in %")
        ? num_(r[idx["Risk in %"]])
        : 0;
    ...
    return {
      // Use Serial No. (column B => index 1) as the row identifier
      id: String(r[1]) || String(i + 2),
      symbol: instrument,
      action: side,
      quantity: qty,
      entryPrice: entry,
...
function doPost(e) {
  ...
  console.log('RowData[0] (ID):', rowData[0]);
  ...
  if (action === 'update') {
    // Sheet header order: [Timestamp, Serial No., Entry date, ...]
    // Match by Serial No. which is column index 1 (B)
    const id = rowData[1];
    ...
    for (let i = 1; i < data.length; i++) {
      console.log('Row', i + 1, 'Serial No. (col 1):', data[i][1]);
    ...
    console.log('No match found, inserting as new row');
    ...
    // Ensure correct column order: [Timestamp, Serial No., Entry date, ...]
    const tsCell = rowData[0];
    const slNoCell = rowData[1];
    if (tsCell === '' || tsCell == null) {
      rowData[0] = timestampStr; // Column 0: Timestamp
    }
    if (slNoCell === '' || slNoCell == null) {
      rowData[1] = serialStr; // Column 1: Serial No.
    }
  ...
  var movers = rows
    ...
    .map(function (r) {
      ...
      if (iTrend >= 0) {
        ...
        trend =
          Number(changePct) < 0 ||
          (typeof changeStr === "string" && changeStr.trim().charAt(0) === "-")
            ? "down"
            : "up";
      }
...
function getTopProfit_(sheetName = "TopProfit") {
  try {
    ...
    const headerMap = {
      ...
      profitPercent: findHeader(header, [
        "profit%",
        "profit %",
        "profitpercent",
        "return%",
    ...
    console.log(
      ...
      headerMap.tradesCount !== -1
        ? header[headerMap.tradesCount]
        : "Not found",
      "at index",
    ...
    rows.slice(0, 5).forEach((row, i) => {
      const tradesVal =
        headerMap.tradesCount !== -1 ? row[headerMap.tradesCount] : undefined;
      console.log(
        `[TopProfit] Row ${i} trades count:`,
        tradesVal,
        typeof tradesVal
      );
    });
    ...
    rows.forEach((row, i) => {
      try {
        const sym =
          headerMap.symbol !== -1 ? String(row[headerMap.symbol] || "") : "";
        const symbolRaw = sym.trim();
        const symbol = canonicalSymbol_(symbolRaw);
        if (!symbol) return;

        const profit =
          headerMap.profit !== -1 ? num_(row[headerMap.profit]) : 0;
        const pct =
          headerMap.profitPercent !== -1
            ? num_(row[headerMap.profitPercent])
            : NaN;
        const tcount =
          headerMap.tradesCount !== -1 ? num_(row[headerMap.tradesCount]) : NaN;
        const avg = headerMap.avgWin !== -1 ? num_(row[headerMap.avgWin]) : NaN;
        const name =
          headerMap.name !== -1 ? String(row[headerMap.name] || "") : "";
        const updRaw =
          headerMap.updatedAt !== -1 ? row[headerMap.updatedAt] : "";
        ...
        if (typeof pct === "number" && !isNaN(pct)) {
          g.profitPercentSum += pct;
          g.profitPercentCount += 1;
        }
        if (typeof avg === "number" && !isNaN(avg)) {
          g.avgWinSum += avg;
          g.avgWinCount += 1;
        }
        if (g.hasTradesCol && typeof tcount === "number" && !isNaN(tcount)) {
          g.tradesSum += tcount;
          g.tradesValueCount += 1;
        }
        if (name && !g.name) g.name = name;
        if (upd) {
          if (!g.updatedAt) g.updatedAt = upd;
          else if (new Date(upd).getTime() > new Date(g.updatedAt).getTime())
            g.updatedAt = upd;
        }
      } catch (err) {
    ...
    const aggregated = Object.keys(groups)
      .map((k) => {
        const g = groups[k];
        const profitPercent =
          g.profitPercentCount > 0
            ? g.profitPercentSum / g.profitPercentCount
            : 0;
        ...
        const tradesCount = g.hasTradesCol
          ? g.tradesValueCount > 0
            ? g.tradesSum
            : g.rowsCount
          : g.rowsCount;
        return {
...
function findHeader(header, keys) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    index = normHeaders.findIndex(
      (h) => h === nk || h.includes(nk) || nk.includes(h)
    );
    if (index !== -1) return index;
  }
  return -1;
...
function canonicalSymbol_(raw) {
  ...
  try {
    ...
    if (compact.indexOf("NIFTYBANK") >= 0 || compact.indexOf("BANKNIFTY") >= 0)
      return "BANKNIFTY";
    if (compact.indexOf("FINNIFTY") >= 0) return "FINNIFTY";
    if (
      compact.indexOf("MIDCPNIFTY") >= 0 ||
      compact.indexOf("MIDCAPNIFTY") >= 0
    )
      return "MIDCPNIFTY";
...
function getTradingDayStats_(sheetName = DEFAULT_SHEET) {
  ...
  try {
    ...
    const dateCol = findHeader(header, [
      "date",
      "timestamp",
      "time",
      "datetime",
    ...
    const pnlCol = findHeader(header, [
      "pnl",
      "profit",
      "net",
      "p&l",
...
function getPortfolioStats_(sheetName = DEFAULT_SHEET) {
  try {
    ...
    const exitDateCol = findHeader(header, [
      "exit date",
      "exitdate",
      "exit_date",
    ]);
    const pnlCol = findHeader(header, ["p&l", "pnl", "profit", "net"]);
    const sideCol = findHeader(header, [
      "side",
      "action",
      "position",
      "trade side",
    ]);
    const currentPriceCol = findHeader(header, [
      "current price",
      "ltp",
      "cmp",
      "last",
    ...
    const instrumentCol = findHeader(header, [
      "instrument",
      "symbol",
      "ticker",
      "scrip",
    ...
    rows.forEach((row) => {
      ...
      const isOpen =
        exitDateCol !== -1
          ...
          ? exitPriceVal === "" ||
            exitPriceVal == null ||
            Number(exitPriceVal) === 0
          : false;

      // Track all instruments (for total unique stocks count)
      const instrument =
        instrumentCol !== -1 ? String(row[instrumentCol] || "").trim() : "";
      if (instrument) {
      ...
      if (isOpen) {
        ...
        if (!isNaN(entry) && !isNaN(qty)) {
          ...
          const usePrice =
            typeof cur === "number" && !isNaN(cur) && cur > 0 ? cur : entry;
    ...
    const summary =
      ss.getSheetByName("Summary") || ss.getSheetByName("Dashboard");
        ...
        if (vals && vals.length > 0) {
          ...
          if (metricIdx !== -1 && valueIdx !== -1) {
            for (let i = 1; i < vals.length; i++) {
              const metric = String(vals[i][metricIdx] || "")
                .trim()
                .toLowerCase();
              if (
                [
...
function num_(v) {
  ...
  s = s.replace(/\u2212|\u2013|\u2014|–|—/g, "-").replace(/\u00A0/g, " "); // NBSP -> space
...
function sendOtp_(email) {
  try {
    ...
    if (row === -1) {
      try {
        console.warn(
          "[sendOtp_] Blocked non-allowed email",
          (email || "").replace(/(^.).*(@.*$)/, "$1***$2")
        );
      } catch {}
    ...
    cache.put(
      metaKey,
      JSON.stringify({ lastSentAt: now, sendCount: (meta.sendCount || 0) + 1 }),
      300
    );
    ...
    var html =
      ...
      '<div style="font-size: 32px; font-weight: 700; text-align: center; color: #1a202c; background-color: #f1f5f9; padding: 15px 20px; border-radius: 8px; margin-bottom: 20px; letter-spacing: 4px;">' +
      code +
      "</div>" +
      ...
      '<p style="font-size: 12px; color: #64748b; margin: 0;">&copy; ' +
      new Date().getFullYear() +
      " Trading Analysis. All rights reserved.</p>" +
      "</div>" +
      "</div>" +
      "</div>";
    try {
      try {
        console.log(
          "[sendOtp_] Attempting MailApp.sendEmail for",
          email.replace(/(^.).*(@.*$)/, "$1***$2")
        );
      } catch {}
      ...
      try {
        console.error("[sendOtp_] Mail send failed:", String(mailErr));
      } catch {}
      return {
        success: false,
        error: "Failed to send email: " + String(mailErr),
      };
    }

    try {
      console.log("[sendOtp_] OTP email dispatched");
    } catch {}
...
function verifyOtp_(email, code) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      var rowEmail = String(data[i][colGmail] || "")
        .trim()
        .toLowerCase();
      if (rowEmail === email) {
        matchedRow = i;
