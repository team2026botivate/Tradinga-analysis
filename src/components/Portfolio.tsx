          ...
          <div className="flex bg-slate-100 rounded-xl p-1">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                view === 'overview' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
              }`}
            >
              Overview
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                view === 'positions' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'
              }`}
            >
              Positions
            </button>
      ...
      {view === 'overview' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Portfolio Performance</h2>
            <div className="h-64 bg-slate-50 rounded-xl flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-slate-500 font-medium">Portfolio Chart</p>
          ...
          {/* Sector Allocation */}
          <div className="bg-white rounded-2xl p-6 border border-slate-200">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Sector Allocation</h2>
                ...
                <div key={index} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm text-slate-600">{sector.value}</p>
                    </div>
                  </div>
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-600">{position.shares} shares</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-slate-900">{position.totalReturn}</p>
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                  <div className="flex items-center space-x-3">
                    <div className={`px-2 py-1 rounded-full text-xs font-bold ${
                      activity.action === 'BUY' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
                    }`}>
                      {activity.action}
                    </div>
                    <div>
                      <p className="font-semibold text-slate-800">{activity.symbol}</p>
                      <p className="text-sm text-slate-600">{activity.shares} shares @ {activity.price}</p>
                    </div>
                  </div>
        ...
        /* Positions Table */
        <div className="bg-white rounded-2xl p-6 border border-slate-200">
          <h2 className="text-xl font-bold text-slate-900 mb-4">Current Positions</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="text-right py-3 px-4 font-semibold text-slate-700">Market Value</th>
                  <th className="text-right py-3 px-4 font-semibold text-slate-700">Total Return</th>
                  <th className="text-right py-3 px-4 font-semibold text-slate-700">Day Change</th>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600">{position.name}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-right font-semibold text-slate-900">{position.shares}</td>
                    <td className="py-3 px-4 text-right text-slate-600">{position.avgCost}</td>
                    <td className="py-3 px-4 text-right font-semibold text-slate-900">{position.currentPrice}</td>
                    <td className="py-3 px-4 text-right font-semibold text-slate-900">{position.marketValue}</td>
